    ...
    where TChange : IChange, new()
{
    ...
    protected override ValueTask InternalUndo(TChange change, CancellationToken cancel) =>
        undo(change, cancel);

    protected override ValueTask InternalRedo(TChange change, CancellationToken cancel) =>
        redo(change, cancel);
}
