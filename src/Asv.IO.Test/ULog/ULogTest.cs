...
public class ULogInfoTokensTest
{
    ...
    public void ReadHeaderWithParams()
    {
        ...
        var paramsDict = new Dictionary<MultiInformationTokenKey, IList<MultiInformationTokenValue>>();
        while (reader.TryRead(ref rdr, out var token))
        {
            ...
            if (!paramsDict.TryGetValue(param.Key, out var value))
            {
                value = new List<MultiInformationTokenValue> { param.Value };
                paramsDict[param.Key] = value; 
                continue;
            }
    ...
    private string ValueToString(MultiInformationTokenValue value)
    {
    ...
    private ReadOnlySpan<char> CharToString(MultiInformationTokenValue value)
    {
    ...
    public void DeserializeToken_Success(byte isContinued, string type, string name, ValueType value)
    {
    ...
    public void DeserializeToken_WrongKeyName(byte isContinued, string type, string name, ValueType value)
    {
    ...
    public void DeserializeToken_NoKeyBytes(byte isContinued, string type, string name, ValueType value)
    {
    ...
    public void DeserializeToken_WrongKeyBytes(byte isContinued, string type, string name, ValueType value)
    {
        Assert.Throws<ULogException>(() =>
        {
    ...
    public void DeserializeToken_NoType(byte isContinued, string type, string name, ValueType value)
    {
        Assert.Throws<ULogException>(() =>
        {
            var readOnlySpan = SetUpTestData(isContinued, type, name, value, 0);
    ...
    private static ValueType InformationTokenValueToValueType(MultiInformationTokenValue value)
    {
        switch (value.Type)
        {
