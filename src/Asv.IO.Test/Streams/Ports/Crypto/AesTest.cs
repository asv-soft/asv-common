...
{
    public static AesCryptoPort CreateAesCryptoPort(PortBase port) =>
        new(
            port,
            new AesCryptoPortConfig
            {
    ...
    public async Task AesTcpClientAndServerDataTest()
    {
        ...
        {
            if (
                aesClient.State.Value == PortState.Connected
                && aesServer.State.Value == PortState.Connected
            )
            {
                break;
            }
