...
{
    public IEnumerable<TId> Path { get; }
    public string ChangeId { get; }
    public Guid DataRefId { get; }
}
        ...
        ISupportUndo<TBase, TId>
{
    ...
    public UndoHistory(TBase owner, IUndoHistoryStore<TId> store)
    {
        ...
        // load from store change info
        _undoStack.PushRange(_store.LoadUndoStack());
        _redoStack.PushRange(_store.LoadRedoStack());
    ...
    public async ValueTask UndoAsync(CancellationToken cancel = default)
    {
        if (_undoStack.TryPop(out var snapshot))
        {
            ...
            _store.LoadChange(snapshot.DataRefId, change);
    ...
    private ValueTask TryAddToHistory(TBase x, UndoEvent<TBase> e)
    {
        ...
        _store.SaveChange(e.Change); // TODO: move serialization to separate thread
        _undoStack.Push(snapshot);
        _redoStack.Clear();
