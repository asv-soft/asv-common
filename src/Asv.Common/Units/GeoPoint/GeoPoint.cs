...
namespace Asv.Common
{
    ...
    /// </summary>
    public readonly struct GeoPoint(double latitude, double longitude, double altitude)
        : IEquatable<GeoPoint>
    {
        ...
        {
            if (
                GeoPointLatitude.TryParse(latitude, out var lat) == false
                || GeoPointLongitude.TryParse(longitude, out var lon) == false
            )
            {
                geoPoint = default;
                return false;
        ...
        public static GeoPoint Random(
            ...
            double minAltitude = -10000.0,
            double maxAltitude = 10000.0
        )
        {
