...
namespace Asv.Common
{
    ...
    {
        public static TextTableBorder Ascii { get; } = new AsciiTableBorderSingle();
        public static TextTableBorder Single { get; } = new SingleTextTableBorder();
        public static TextTableBorder Double { get; } = new DoubleTextTableBorder();
        public static TextTableBorder Empty { get; } = new EmptyTextTableBorder();
        ...
        public static string TableBegin(TextTableBorder border, IEnumerable<int> values)
        {
            return border.TopLeft
                + string.Join(
                    ...
                    values.Select(i => string.Empty.PadLeft(i, border.TopCenter))
                )
                + border.TopRight;
        }
        ...
        private static string TableEndRow(TextTableBorder border, IEnumerable<int> values)
        {
            return border.Middle2Left
                + string.Join(
                    border.Middle2Intersection.ToString(),
                    values.Select(i => string.Empty.PadLeft(i, border.Middle2Center))
                )
                + border.Middle2Right;
        }
        ...
        public static string TableEnd(TextTableBorder border, IEnumerable<int> values)
        {
            return border.BottomLeft
                + string.Join(
                    border.BottomIntersection.ToString(),
                    values.Select(i => string.Empty.PadLeft(i, border.BottomCenter))
                )
                + border.BottomRight;
        }
        ...
        )
        {
            PrintTableFromObject(
                ...
                maxLength,
                items,
                properties.Select(e => GetPropertyInfo(e).Name)
            );
        }
        ...
        )
        {
            PrintTableFromObject(
                ...
                maxLength,
                items,
                properties.ToDictionary(s => s, s => s)
            );
        }
...
#pragma warning disable CS8714 // The type cannot be used as type parameter in the generic type or method. Nullability of type argument doesn't match 'notnull' constraint.
            PrintTableFromObject(
                ...
                maxLength,
                items,
                properties.ToDictionary(p => t.GetProperty(p.Key), p => p.Value)
            );
...
#pragma warning restore CS8620 // Argument cannot be used for parameter due to differences in the nullability of reference types.
        ...
        )
        {
            ...
            var width = Enumerable
                ...
                .GroupBy(
                    i => i % columns,
                    i => headerWithRows[i / columns][i % columns].TrimToMaxLength(maxLength)
                )
                .Select(g => g.Max(s => s?.Length ?? 0))
                .Select(i => i + (padding * 2))
                .ToArray();
                    ...
                    headerWithRows
                        ...
                        .Select(
                            (s, i) =>
                                ...
                                + s.TrimToMaxLength(maxLength)?.PadRight(width[i] - padding)
                        )
                )
            );
        ...
        public static void PrintTable(
            ...
            int padding,
            int maxLength,
            IEnumerable<IEnumerable<string>> headerWithRows
        )
        {
        ...
        )
        {
            ...
            var values = items
                .Select(i =>
                    props
                        .Select(prop =>
                            prop.Key.GetValue(i)?.ToString() ?? throw new NullReferenceException()
                        )
                        .ToArray()
                )
                .ToList();
        ...
        )
        {
            ...
            if (member == null)
            {
                ...
                if (propertyLambda.Body is UnaryExpression expression)
                {
