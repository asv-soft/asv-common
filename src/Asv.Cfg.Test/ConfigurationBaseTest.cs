    ...
    where T : IConfiguration
{
    ...
    public void Check_CustomSaveInterface_Success()
    {
        ...
        var origin = new CustomTestClass { Value = fixture.Create<string>() };
        cfg.Set(origin);
    ...
    public void Check_Available_Parts_Value()
    {
        ...
        var actualResult = cfg
            .AvailableParts.OrderBy(x => x)
            .Where(x => exclude.Contains(x) == false)
            .ToArray(); // items can be reordered in any way, so we need to sort them
    ...
    public void Check_For_Multiple_Threads_To_Write_In_Same_Field()
    {
        ...
        for (var i = 0; i < threads.Length; i++)
        {
            ...
            threads[i] = new Thread(() =>
                cfg.Set(new TestClass() { Name = $"TestMultiThread{j}" })
            );
            threads[i].Start();
        }
    ...
    public void Check_For_Multiple_Threads_To_Write_In_Multiple_Fields()
    {
        ...
        threads[0] = new Thread(() =>
            cfg.Set("one", new TestMultiThreadClassOne() { Name = "First" })
        );
        threads[1] = new Thread(() =>
            cfg.Set("two", new TestMultiThreadClassTwo() { Name = "Second" })
        );
        threads[2] = new Thread(() =>
            cfg.Set("three", new TestMultiThreadClassThree() { Name = "Third" })
        );
        threads[3] = new Thread(() =>
            cfg.Set("four", new TestMultiThreadClassFour() { Name = "Fourth" })
        );
