    ...
    public class CustomSerialPort : PortBase
    {
        ...
        protected override void InternalStart()
        {
            using (_sync.Lock())
            {
                ...
                _serial.Open();
                _readingTimer = TimeProvider.CreateTimer(
                    TryReadData,
                    null,
        ...
        private void TryReadData(object? state)
        {
            ...
            try
            {
                ...
                using (_sync.Lock())
                {
                    if (_serial == null || _serial.BytesToRead == 0 || _serial.IsOpen == false)
                    {
                        goto exit;
                    }
        ...
        protected override async ValueTask DisposeAsyncCore()
        {
            if (_serial != null)
            {
                await CastAndDispose(_serial);
            }

            if (_readingTimer != null)
            {
                await CastAndDispose(_readingTimer);
            }
