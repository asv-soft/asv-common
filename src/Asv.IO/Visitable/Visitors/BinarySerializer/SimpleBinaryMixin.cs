...
{
    public static int GetSize<T>(T value, bool skipUnknown = false)
        where T : IVisitable
    {
    ...
    public static void Serialize<T>(T value, IBufferWriter<byte> buffer, bool skipUnknown = false)
        where T : IVisitable
    {
    ...
    public static void Deserialize<T>(
        T value,
        ref ReadOnlyMemory<byte> buffer,
