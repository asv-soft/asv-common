...
public partial class Schema
{
    ...
    {
        private readonly ImmutableArray<Field>.Builder _fields =
            ImmutableArray.CreateBuilder<Field>();
        ...
        public Schema Build()
        {
            return new Schema(
                _fields.ToImmutable(),
                _metadata?.ToImmutable() ?? ImmutableDictionary<string, string>.Empty
            );
        }

        public override string ToString() =>
            $"{nameof(Schema)}: Num fields={_fields.Count}, Num metadata={_metadata?.Count ?? 0}";
    }
}
