...
) : StructType(fields)
{
    ...
    public static Schema Empty =>
        new Schema(ImmutableArray<Field>.Empty, ImmutableDictionary<string, string>.Empty);
    ...
    public Schema InsertField(int fieldIndex, Field newField) =>
        new(Fields.Add(newField), Metadata);

    public Schema SetField(int fieldIndex, Field newField) =>
        new(Fields.SetItem(fieldIndex, newField), Metadata);
}
