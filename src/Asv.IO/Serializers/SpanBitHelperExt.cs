...
namespace Asv.IO
{
    public static partial class SpanBitHelper
    {
        ...
        public static void SetFixedPointS3Bit(Span<byte> buffer, ref int bitIndex, double value)
        {
            ...
            {
                SetBitS(buffer, ref bitIndex, 3, FixedPointS3Nan);
                return;
            }

            if (double.IsNegativeInfinity(value))
            {
                SetBitS(buffer, ref bitIndex, 3, FixedPointS3NegativeInf);
                return;
            }

            if (double.IsPositiveInfinity(value))
            {
                SetBitS(buffer, ref bitIndex, 3, FixedPointS3PositiveInf);
                return;
            }
            ...
            switch (intValue)
            {
                ...
                case < FixedPointS3Min:
                    ...
                    break;
                default:
                    SetBitS(buffer, ref bitIndex, 3, intValue);
                    break;
            }
        }
