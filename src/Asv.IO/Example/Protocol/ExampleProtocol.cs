...
public static class ExampleProtocol
{
    ...
    public static void RegisterExampleProtocol(
        this IProtocolParserBuilder builder,
        Action<IProtocolMessageFactoryBuilder<ExampleMessageBase, byte>>? configure = null
    )
    {
        var factory = new ProtocolMessageFactoryBuilder<ExampleMessageBase, byte>(Info);
        factory.Add<ExampleMessage1>().Add<ExampleMessage2>().Add<ExampleMessage3>();
