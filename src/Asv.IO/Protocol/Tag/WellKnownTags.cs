...
public static class WellKnownTags
{
    ...
    public static void SetConnectionId(this ISupportTag src, string id)
    {
        src.Tags[ConnectionIdTag] = id;
    }

    public static string? GetConnectionId(this ISupportTag src)
    {
        return (string?)src.Tags[ConnectionIdTag];
    }
}
...
public struct ProtocolTags : IList<KeyValuePair<string, object?>>, IReadOnlyList<KeyValuePair<string, object?>>
{
        ...
        set
        {
            if (_overflowTags is null)
            {
                for (var i = 0; i < _tagsCount; i++)
                {
                    if (this[i].Key == name)
                    {
                        this[i] = new KeyValuePair<string, object?>(name, value);
                        return;
                    }
                }
            }
            else
            {
                for (var i = 0; i < _tagsCount; i++)
                {
                    if (_overflowTags[i].Key == name)
                    {
                        _overflowTags[i] = new KeyValuePair<string, object?>(name, value);
                        return;
                    }
                }
            }
    ...
    {
        private readonly ProtocolTags _tagList;
        private int _index;

        internal Enumerator(in ProtocolTags tagList)
        {
            _index = -1;
            _tagList = tagList;
        }
    ...
    public void AddRange(IReadOnlyList<KeyValuePair<string, object?>> messageTags)
    {
...
public sealed class ProtocolTags
{
    ...
    {
        return _tagList.Count == 0 ? string.Empty : string.Join(", ", _tagList.Select(x => $"{x.Key}={x.Value}"));
    }
}*/