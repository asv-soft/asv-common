...
public sealed class VirtualConnection: IVirtualConnection
{
    ...
    public VirtualConnection(Func<IProtocolMessage, bool>? clientToServerFilter,Func<IProtocolMessage, bool>? serverToClientFilter, IProtocolContext context, bool printMessages = true)
    {
        ...
        if (printMessages)
        {
            ...
            var scInx = 0;
            _sub3 = Client.OnRxMessage.Subscribe(serverToClient,(x,log) => log.ZLogTrace( $"[{Interlocked.Increment(ref scInx):000}] {context.PrintMessage(x)}"));
            var clientToServer = context.LoggerFactory.CreateLogger("CLIENT=>SERVER");
            var csInx = 0;
            _sub4 = Server.OnRxMessage.Subscribe(clientToServer,(x,log) => log.ZLogTrace($"[{Interlocked.Increment(ref csInx):000}] {context.PrintMessage(x)}"));    
        }
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (_sub3 != null) await CastAndDispose(_sub3);
        if (_sub4 != null) await CastAndDispose(_sub4);
        return;
