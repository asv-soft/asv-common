...
{
    public static void SetClientToServerFilterByType<TMessage>(this IVirtualConnection src, Func<TMessage, bool> filter)
        where TMessage:IProtocolMessage
    {
        src.SetClientToServerFilter(message => message is TMessage m && filter(m));
    }

    public static void SetServerToClientFilter<TMessage>(this IVirtualConnection src, Func<TMessage, bool> filter)
    {
        src.SetClientToServerFilter(message => message is TMessage m && filter(m));
    }
}
